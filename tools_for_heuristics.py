...
from tools_for_graph_nodes import ListNodes, Node
from tools_for_graph_nodes import build_graph_nodes, get_dims_from_pic
...
def parallel_update_h_table(node, nodes, nodes_dict, map_dim, h_dict, node_index, **kwargs):
    print(f'[HEURISTIC]: Thread {node_index} started.')
    h_table = build_heuristic_for_one_target(node, nodes, nodes_dict, map_dim, **kwargs)
...
#     h_dict = {}
#     h_reset_nodes(nodes, target_nodes)
...
def h_reset_nodes(nodes, target_nodes=None):
    _ = [node.reset(target_nodes) for node in nodes]


def build_heuristic_for_one_target(target_node: Node, nodes: List[Node], nodes_dict: Dict[str, Node], map_dim: Tuple[int, int], **kwargs) -> np.ndarray:
    # print('Started to build heuristic...')
    copy_nodes = nodes
...
def main():
    ...
    h_dict = parallel_build_heuristic_for_entire_map(nodes, nodes_dict, map_dim, img_dir=img_dir, path='logs_for_heuristics')
    pprint(f'{len(h_dict)=}')
